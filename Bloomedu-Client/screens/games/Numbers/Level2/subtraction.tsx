...
export default function subtraction({ navigation }: any) {
  const route = useRoute();
  const { child, gameSequence, currentGameIndex, categoryTitle }: any = route.params || {};

  const [data, setData] = useState<SubtractionData | null>(null);
  ...
  const generateSubtraction = (): SubtractionData => {
    // Çıkarma için her zaman n1 >= n2 olmalı
    const n1 = Math.floor(Math.random() * 8) + 2; // 2-10 arası
    const n2 = Math.floor(Math.random() * n1) + 1; // 1 ile n1 arası
    const correct = n1 - n2;
  ...
  const startNewRound = () => {
    const newData = generateSubtraction();
    setData(newData);
    setSelectedAnswer(null);
    setIsCorrect(null);
    
    setTimeout(() => {
      Tts.speak(`${newData.num1} minus ${newData.num2}. How many are left?`);
    }, 600);
  };
  ...
  const handleAnswerPress = (answer: number) => {
    ...
    if (correct) {
      ...
      Tts.stop();
      Tts.speak(`Great! ${data.num1} minus ${data.num2} is ${data.correctAnswer}.`);
      ...
      Tts.stop();
      Tts.speak('Try again. Think about taking away!');
      setTimeout(() => {
        setSelectedAnswer(null);
        setIsCorrect(null);
      }, 1500);
  ...
  const handleGameCompletion = () => {
    ...
    gameNav.showCompletionMessage(score, MAX_ROUNDS, '💎 Subtraction Expert!');
  };
    ...
    <View style={styles.container}>
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Subtraction Fun</Text>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.equationArea}>
            <View style={[styles.numCard, { backgroundColor: '#FF5252' }]}>
              <Text style={styles.numText}>{data.num1}</Text>
            </View>

            <Text style={styles.operatorText}>−</Text>

            <View style={[styles.numCard, { backgroundColor: '#FFD740' }]}>
          ...
          <Text style={styles.instruction}>How many are left?</Text>
...
const styles = StyleSheet.create({
  ...
  backText: { color: '#FF5252', fontWeight: 'bold', fontSize: 16 },
  title: { fontSize: 18, fontWeight: 'bold', color: '#333' },
  scoreBox: { backgroundColor: '#FFD740', paddingHorizontal: 15, paddingVertical: 8, borderRadius: 20 },
  scoreText: { color: '#333', fontWeight: 'bold' },
  ...
  progressFill: { height: '100%', backgroundColor: '#FF5252' },
  ...
  operatorText: { fontSize: 45, fontWeight: 'bold', color: '#576574', marginHorizontal: 10 },
  resultPlaceholder: { width: 80, height: 100, backgroundColor: '#fff', borderRadius: 15, borderStyle: 'dashed', borderWidth: 3, borderColor: '#FF5252', justifyContent: 'center', alignItems: 'center' },
  placeholderText: { fontSize: 45, fontWeight: '900', color: '#FF5252' },
  instruction: { fontSize: 18, color: '#576574', marginBottom: 20, fontWeight: '600' },
  optionsContainer: { flexDirection: 'row', gap: 20 },
  optionBtn: { width: 80, height: 80, backgroundColor: '#fff', borderRadius: 15, justifyContent: 'center', alignItems: 'center', elevation: 4, borderWidth: 2, borderColor: '#eee' },
  optionText: { fontSize: 32, fontWeight: 'bold', color: '#2d3436' },
  correctBtn: { borderColor: '#4CAF50', backgroundColor: '#E8F5E9' },
  wrongBtn: { borderColor: '#FF5252', backgroundColor: '#FFF0F0' },
  successMessage: { position: 'absolute', top: height / 4, alignSelf: 'center', backgroundColor: '#4CAF50', paddingHorizontal: 30, paddingVertical: 15, borderRadius: 30, zIndex: 999, elevation: 10 },
